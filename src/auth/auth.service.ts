...
export class AuthService {
    ...
    async getTokensPair(user: User){
        let expiredAt = new Date(Date.now() + 3600000);
        let token = uuidv4();
    ...
    async getNewTokensPair(@Req() req: UserRequestInterface, refreshTokenDto: RefreshTokenDto): Promise<LoginResponseInterface> {
        let expiredAt = new Date(Date.now() + 3600000);
        let token = uuidv4();
        const refreshToken = await this.refreshTokenModel.findOne(refreshTokenDto).populate({
            path: 'user'
        }).exec();
        if(!refreshToken){
            throw new HttpException('incorrect refresh token', HttpStatus.FORBIDDEN);
        }
        await this.refreshTokenModel.updateOne(refreshTokenDto, {
            refreshToken: token,
            expirationDate: expiredAt,
        }).exec();
        return {
